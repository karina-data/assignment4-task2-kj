...
# adds a caption
      caption = "Table 1. Summary statistics for juvenile male and female snowshoe hare weights (g) at Bonanza Creek 1998-2012 (Data: Kielland et al. 2017).") %>% 
...
Female mean snowshoe hare weight (`r hare_weight_stats$mean_weight[1]` $\pm$ `r hare_weight_stats$sd_weight[1]`) was found to be significantly lower than male mean hare weight (`r hare_weight_stats$mean_weight[2]` $\pm$ `r hare_weight_stats$sd_weight[2]`) (mean $\pm$ standard deviation) (t-test: t(`r round(weight_ttest$parameter, 1)`) = `r round(weight_ttest$statistic, 3)`; p = `r round(weight_ttest$p.value, 3)`). However, both standard deviation and sample size (n = `r hare_weight_stats$sample_size[1]` for females and n = `r hare_weight_stats$sample_size[2]` for males) were large.
...
```
The slope (`r round(hare_lm_tidy$estimate[2], 2)`) of Equation 1 indicates that for every 1 mm increase in hind foot length, we would expect the hare weight to increase by `r round(hare_lm_tidy$estimate[2], 2)` grams. 

The results of the regression are suspect due to the lack of meeting the assumptions for a linear regression model, most notably the heteroscedasticity of the residuals and the lack of normality of the residuals in the Q-Q plot. The linearity and independence of errors assumptions seemed to be met. Additionally, since the model only explains `r (round(hare_lm_glance$r.squared, 3)*100)`% of the variance for hare weight, it is likely that other variables are influencing the model that are not analyzed here. Logically, it also seems likely that hind foot length alone is not an appropriate predictor of hare weight. 
